...
const DifferenceList = ({ result, onDifferenceClick, onFilterChange }) => {
  ...
  useEffect(() => {
    const fetchAllDifferences = async () => {
      ...
      try {
        // Collect differences from all pages
        const allDiffs = [];
        let pagesDone = 0;
        
        // Process each page that has differences
        for (const page of result.pageDifferences) {
          // Skip pages that only show structural differences
          if (page.onlyInBase || page.onlyInCompare) {
            allDiffs.push({
              id: `page-${page.pageNumber}`,
              type: 'structure',
              page: page.pageNumber,
              description: page.onlyInBase 
                ? `Page ${page.pageNumber} only exists in base document` 
                : `Page ${page.pageNumber} only exists in comparison document`,
              severity: 'critical'
            });
            continue;
          }
          
          // Skip pages without differences
          ...
          // Fetch detailed differences for this page
          try {
            ...
            if (details.baseDifferences) {
              // Add page number to each difference
              const pageDiffs = details.baseDifferences.map(diff => ({
                ...diff,
                page: page.pageNumber
              }));
            ...
            console.error(`Error fetching differences for page ${page.pageNumber}:`, err);
          }
        ...
        // Process metadata differences
        if (result.metadataDifferences) {
          const metadataDiffs = Object.entries(result.metadataDifferences).map(([key, diff]) => ({
            id: `metadata-${key}`,
            type: 'metadata',
            key: diff.key,
            baseValue: diff.baseValue,
            compareValue: diff.compareValue,
            changeType: diff.onlyInBase ? 'deleted' : diff.onlyInCompare ? 'added' : 'modified',
            description: `Metadata difference in "${diff.key}"`,
            severity: 'minor'
          }));
        ...
        setFilteredDifferences(allDiffs);
        setLoading(false);
        
        // Initialize expanded state for all pages with differences
        const expandedState = {};
        const pages = new Set(allDiffs.map(diff => diff.page).filter(Boolean));
        pages.forEach(page => {
          expandedState[page] = true; // Default to expanded
        });
        setExpandedPages(expandedState);
      } catch (err) {
        console.error('Error fetching all differences:', err);
        setError('Failed to load all differences');
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (!allDifferences.length) return;
    
    // Filter by type
    let filtered = allDifferences;
    
    if (typeFilter !== 'all') {
      filtered = filtered.filter(diff => diff.type === typeFilter);
    }
    ...
    if (severityFilter !== 'all') {
      filtered = filtered.filter(diff => {
        if (severityFilter === 'critical') {
          return diff.severity === 'critical';
        } else if (severityFilter === 'major') {
          return ['critical', 'major'].includes(diff.severity);
        } else if (severityFilter === 'minor') {
          return ['critical', 'major', 'minor'].includes(diff.severity);
        }
        return true;
      });
    }
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(diff => {
        // Search in various properties depending on difference type
        return (
          (diff.description && diff.description.toLowerCase().includes(term)) ||
          (diff.baseText && diff.baseText.toLowerCase().includes(term)) ||
          (diff.compareText && diff.compareText.toLowerCase().includes(term)) ||
          (diff.key && diff.key.toLowerCase().includes(term)) ||
          (diff.baseValue && String(diff.baseValue).toLowerCase().includes(term)) ||
          (diff.compareValue && String(diff.compareValue).toLowerCase().includes(term))
        );
      });
    }
    ...
    // Update parent component filters if needed
    if (onFilterChange) {
      onFilterChange({
        differenceTypes: typeFilter === 'all' ? 
          ['text', 'image', 'font', 'style', 'metadata', 'structure'] : 
          [typeFilter],
        minSeverity: severityFilter,
        searchTerm
      });
    }
  }, [allDifferences, typeFilter, severityFilter, searchTerm, onFilterChange]);
