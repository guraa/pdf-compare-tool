...
public class EnhancedPageMatcher {
    ...
    private double highSimilarityThreshold = 0.8;     // For direct matches
    private double mediumSimilarityThreshold = 0.5;   // For probable matches
    private double lowSimilarityThreshold = 0.3;      // For possible matches
    ...
    private List<PageFingerprint> generateFingerprints(List<PDFPageModel> pages, String sourceType) {
        ...
        for (int i = 0; i < pages.size(); i++) {
            ...
            PageFingerprint fingerprint = guraa.pdfcompare.service.FingerprintAdapter.createEnhancedFingerprint(page, sourceType, i);
    ...
    private double[][] calculateSimilarityMatrix(List<PageFingerprint> baseFingerprints,
        ...
        for (int i = 0; i < baseSize; i++) {
            for (int j = 0; j < compareSize; j++) {
                ...
                if (guraa.pdfcompare.service.FingerprintAdapter.getTextHash(baseFingerprints.get(i)) != 0 &&
                        guraa.pdfcompare.service.FingerprintAdapter.getTextHash(baseFingerprints.get(i)) ==
                                guraa.pdfcompare.service.FingerprintAdapter.getTextHash(compareFingerprints.get(j))) {
                    matrix[i][j] = 1.0;
                    continue;
                }
    ...
    private double calculateTextSimilarity(PageFingerprint base, PageFingerprint compare) {
        String baseText = guraa.pdfcompare.service.FingerprintAdapter.getText(base);
        String compareText = guraa.pdfcompare.service.FingerprintAdapter.getText(compare);
        ...
        double jaccardSimilarity = calculateJaccardSimilarity(
                guraa.pdfcompare.service.FingerprintAdapter.getSignificantWords(base),
                guraa.pdfcompare.service.FingerprintAdapter.getSignificantWords(compare));
    ...
    private double calculateStructuralSimilarity(PageFingerprint base, PageFingerprint compare) {
        ...
        Map<String, Integer> baseFonts = guraa.pdfcompare.service.FingerprintAdapter.getFontDistribution(base);
        Map<String, Integer> compareFonts = guraa.pdfcompare.service.FingerprintAdapter.getFontDistribution(compare);

        if (!baseFonts.isEmpty() && !compareFonts.isEmpty()) {
            similarity += calculateFontDistributionSimilarity(baseFonts, compareFonts);
            componentCount++;
        }

        // Compare element counts
        int baseElements = guraa.pdfcompare.service.FingerprintAdapter.getElementCount(base);
        int compareElements = guraa.pdfcompare.service.FingerprintAdapter.getElementCount(compare);

        if (baseElements > 0 && compareElements > 0) {
            double countRatio = (double)Math.min(baseElements, compareElements) /
                    Math.max(baseElements, compareElements);
            similarity += countRatio;
            componentCount++;
        }

        // Compare text position distributions if available
        List<Double> basePositions = guraa.pdfcompare.service.FingerprintAdapter.getTextPositions(base);
        List<Double> comparePositions = guraa.pdfcompare.service.FingerprintAdapter.getTextPositions(compare);

        if (!basePositions.isEmpty() && !comparePositions.isEmpty()) {
            similarity += calculateTextPositionSimilarity(basePositions, comparePositions);
            componentCount++;
        }
    ...
    private double calculateStyleSimilarity(PageFingerprint base, PageFingerprint compare) {
        ...
        if (!baseFonts.isEmpty() && !compareFonts.isEmpty()) {
            return calculateFontDistributionSimilarity(baseFonts, compareFonts);
        }
        return 0.0;
    }
    ...
    private double calculateImageSimilarity(PageFingerprint base, PageFingerprint compare) {
        ...
        boolean baseHasImages = guraa.pdfcompare.service.FingerprintAdapter.hasImages(base);
        boolean compareHasImages = guraa.pdfcompare.service.FingerprintAdapter.hasImages(compare);
        ...
        if (baseHasImages == compareHasImages) {
            ...
            int baseImageCount = guraa.pdfcompare.service.FingerprintAdapter.getImageCount(base);
            int compareImageCount = guraa.pdfcompare.service.FingerprintAdapter.getImageCount(compare);
